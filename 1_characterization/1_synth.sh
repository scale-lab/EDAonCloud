#!/bin/sh
if [ "$#" -lt 3 ]; then
    echo "Usage: ./1_synth.sh RUN_ID CPU_COUT CPU_LIST" >&2
    exit 1
fi
...
# Synthesis script (depends on the tool used)
TCL_FILE=1_synth.tcl
...
perf stat -r 3 -o $RUN_ID-synth-$CPU_COUNT.perf.data -e $PC -C $CPUS taskset -c $CPUS $run_command
