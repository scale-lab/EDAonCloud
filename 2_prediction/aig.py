...
def _aag_to_graph(aag_file, levels=0, v_length=64, run_simulation=False):
    ...
    for node, (src1, src2) in ands.items():
        if src1 % 2 == 0:
            ...
            g.add_edge(src1, node, data={'inv': torch.tensor([0])})
        else:
            # source is an INV gate
            g.add_edge(src1 - 1, node, data={'inv': torch.tensor([1])})
        
        if src2 % 2 == 0:
            # source is an AND gate
            g.add_edge(src2, node, data={'inv': torch.tensor([0])})
        else:
            # source is an INV gate
            g.add_edge(src2 - 1, node, data={'inv': torch.tensor([1])})
